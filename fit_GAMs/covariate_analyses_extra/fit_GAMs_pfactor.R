...
# 1) run gam.smooth.predict.covariateinteraction to look at developmental trajectory for 10th and 90th p-factor percentiles (as well as for internalizing, externalizing, and attention)
# 2) run gam.fit.covariate to look at the main effect of pfactor  
...
dataset = args[1]
scalar = args[2]
print(paste("Fitting age by p-factor GAMs for", dataset))
...
run_gam.smooth.predict.covariateinteraction <- function(gam_df, smooth.var, int.var, covs, k, set.fx, filename, age1, age2){
  ...
  smooth.fittedvals.lowerpercentile$pfactor <- c("low")
  ...
  smooth.fittedvals.upperpercentile$pfactor <- c("high")

  smooth.fittedvals.bypfactor <- rbind(smooth.fittedvals.lowerpercentile, smooth.fittedvals.upperpercentile)
  write.csv(smooth.fittedvals.bypfactor, sprintf("%1$s/%2$s_GAM_ageby%3$s_interaction.csv", GAM_outputs_dir, dataset, filename), quote = F, row.names =F)
}

# 2) run gam.fit.covariate to look at the main effect of pfactor (sensitivity analysis)
run_gam.fit.covariate <- function(gam_df, smooth.var, covariate.interest, covariates.noninterest, k, set.fx, filename){
  pfactor.maineffects <- mclapply(tract_node_labels, 
  ...
  pfactor.maineffects <- do.call(rbind, pfactor.maineffects)
  write.csv(pfactor.maineffects, sprintf("%1$s/%2$s_GAM_%3$s_maineffects.csv", GAM_outputs_dir, dataset, filename), quote = F, row.names =F)
}
...
################################################################# 
# Merge demographics and qc with tract profiles data and pfactor
################################################################# 

# load pfactor tsv
pfactor <- fread(sprintf("/cbica/projects/luo_wm_dev/input/%s/sample_selection_files/%s_pfactor.tsv", dataset, dataset))
pfactor$sub <- paste0("sub-", pfactor$participant_id)
demographics <- left_join(demographics, pfactor[,c("sub", "p_factor_mcelroy_harmonized_all_samples", "internalizing_mcelroy_harmonized_all_samples", "externalizing_mcelroy_harmonized_all_samples", "attention_mcelroy_harmonized_all_samples")], by = "sub")
...
gam_df <- na.omit(gam_df) # exclude subjects if missing p-factor data (1 person in PNC, 85 in HBN)

# check pfactor and mean_fd
cor.test(gam_df$p_factor_mcelroy_harmonized_all_samples, gam_df$mean_fd, method = c("pearson")) #PNC: rho = 0.02553317, p-value = 0.3975; HBN: rho = 0.02246439, p-value = 0.5186 
...
# 1) run gam.smooth.predict.covariateinteraction to look at developmental trajectory for 10th and 90th p-factor percentiles
# p-factor
int.var = "p_factor_mcelroy_harmonized_all_samples"
filename = "pfactor"
run_gam.smooth.predict.covariateinteraction(gam_df, smooth.var, int.var, covs, k, set.fx, filename, age1, age2)

# internalizing
int.var = "internalizing_mcelroy_harmonized_all_samples"
filename = "internalizing"
run_gam.smooth.predict.covariateinteraction(gam_df, smooth.var, int.var, covs, k, set.fx, filename, age1, age2)

# externalizing
int.var = "externalizing_mcelroy_harmonized_all_samples"
filename = "externalizing"
run_gam.smooth.predict.covariateinteraction(gam_df, smooth.var, int.var, covs, k, set.fx, filename, age1, age2)

# attention
int.var = "attention_mcelroy_harmonized_all_samples"
filename = "attention"
run_gam.smooth.predict.covariateinteraction(gam_df, smooth.var, int.var, covs, k, set.fx, filename, age1, age2)
...
# 2) run gam.fit.covariate to look at the main effect of pfactor 
# p-factor
covariate.interest = "p_factor_mcelroy_harmonized_all_samples"
...
# internalizing
covariate.interest = "internalizing_mcelroy_harmonized_all_samples"
covariates.noninterest = "mean_fd + sex"
filename = "internalizing"
run_gam.fit.covariate(gam_df, smooth.var, covariate.interest, covariates.noninterest, k, set.fx, filename)

# externalizing
covariate.interest = "externalizing_mcelroy_harmonized_all_samples"
covariates.noninterest = "mean_fd + sex"
filename = "externalizing"
run_gam.fit.covariate(gam_df, smooth.var, covariate.interest, covariates.noninterest, k, set.fx, filename)

# attention
covariate.interest = "attention_mcelroy_harmonized_all_samples"
covariates.noninterest = "mean_fd + sex"
filename = "attention"
run_gam.fit.covariate(gam_df, smooth.var, covariate.interest, covariates.noninterest, k, set.fx, filename)

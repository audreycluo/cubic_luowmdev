...
run_gam.fit.smooth <- function(gam_df, smooth.var, covs, k, set.fx) {
  ...
  write.csv(GAM_dev_measures, sprintf("%1$s/%2$s_GAM_dev_measures_low_p.csv", GAM_outputs_dir, dataset), quote = F, row.names =F)
}
...
run_gam.derivatives <- function(gam_df, smooth.var, covs, k, set.fx, num.draws, num.pred, credible.interval) {
    ...
    write.csv(smooth.derivatives, sprintf("%1$s/%2$s_GAM_derivatives_low_p.csv", GAM_outputs_dir, dataset), quote = F, row.names =F)
}
...
run_gam.estimate.smooth <- function(gam_df, smooth.var, covs, k, set.fx, num.draws, num.pred, start_age, end_age){
  ...
  write.csv(smooth.centered, sprintf("%1$s/%2$s_GAM_smoothcentered_low_p.csv", GAM_outputs_dir, dataset), quote = F, row.names =F)
}
...
run_gam.smooth.predict <- function(gam_df, smooth.var, covs, k, set.fx, num.pred, start_age, end_age){
  ...
  write.csv(smooth.fittedvals, sprintf("%1$s/%2$s_GAM_smooth_fittedvalues_low_p.csv", GAM_outputs_dir, dataset), quote = F, row.names =F)
}
...
low_p <- read.table("/cbica/projects/luo_wm_dev/input/HBN/subject_list/HBN_low_p_sample.txt", header=T)
tract_profiles_long <- tract_profiles_long[tract_profiles_long$sub %in% low_p$x,]   

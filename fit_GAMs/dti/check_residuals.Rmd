...
fit_tractwise_gamm_re <- function(tract_name, scalar) {
  ...
  te_gam <- gamm(as.formula(sprintf("%s ~ 
...
#fit_tractwise_bam <- function(tract_name, scalar) {
 ...
 # s(nodeID, k = 24, fx = F, bs = 'cr') + 
 # ti(age, nodeID, k=c(3, 24), fx = F) +  
 # s(nodeID, by = subjectID) + sex + mean_fd", scalar)), data = df, method = "fREML", discrete = TRUE) 
...
fit_tractwise_bam_test <- function(tract_name, scalar) {
  df <- gam_df %>% filter(tract_hemi == tract_name)
  df$subjectID <- as.factor(df$subjectID)
  te_gam <- bam(as.formula(sprintf("%s ~ 
  s(age, k = 3, fx = T, bs = 'cr') + 
...
tractwise_bam_test <- lapply(unique(gam_df$tract_hemi), fit_tractwise_bam_test, scalar="dti_md")
names(tractwise_bam_test) <- unique(gam_df$tract_hemi)
saveRDS(tractwise_bam_test, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_bam_test.RData")
```
...
`gamm(dti_md ~ s(age, k = 3, fx = T, bs = 'cr') + s(nodeID, k = 24, fx = F, bs = 'cr') + ti(age, nodeID, k=c(3, 24), fx = F) + sex + mean_fd, random = list(subjectID=~1), data = df)`s
...
plot_compare_residuals <- function(tract, num_subjects) { 
  ...
  gam_plot <- ggplot(gam_df_toplot[gam_df_toplot$subjectID %in% subjects_to_plot, ], aes(x = nodeID, y = residuals_gam, color = factor(subjectID))) +
    ...
    labs(title = "Raw Residual Curves from Model 1 \ngam: s(age, k = 3, fx = T, bs = 'cr') + \ns(nodeID, k = 24, fx = F, bs = 'cr') + \nti(age, nodeID, k=c(3, 24), fx = F) + \ns(subjectID, bs = 're') + sex + mean_fd", x = "Node ID", y = "Residuals")  
...
# Residual plots for each type of model for each tract (showing 15 subjects)

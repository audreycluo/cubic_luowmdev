...
tract_profiles <- fread("/cbica/projects/luo_wm_dev/input/HCPD/derivatives/HCPD_tractprofiles/all_subjects/collated_tract_profiles_HCPD_reoriented.tsv")
...
```{r take subset of 20 subjects, cache=TRUE}
#subjects <- read.table("/cbica/projects/luo_wm_dev/input/HCPD/subject_list/HCPD_tempsubject_list_N572.txt")
subjects <- read.csv("/cbica/projects/luo_wm_dev/input/HCPD/sample_selection_files/HCPD_WMDev_TempSampleDemographics_N572_age8to22.csv")
subjects <- subjects %>% rename(subjectID = sub) %>% select(subjectID,site)
subject_subset <- subjects[c(1:20),]
#gam_df <- gam_df %>% filter(subjectID %in% subject_subset) # fit GAMs on a subset of 20 participants
gam_df <- merge(gam_df, subject_subset)

subject_subset <- subject_subset$subjectID
...
fit_tractwise_gam_re <- function(tract_name, scalar) {
  ...
  s(nodeID, k = 24, fx = FALSE, bs = 'cr', by = site) + site +
...
fit_tractwise_bam_no_re <- function(tract_name, scalar) {
  df <- gam_df %>% filter(tract_hemi == tract_name)
  df$subjectID <- as.factor(df$subjectID)
  df$site <- as.factor(df$site)
  ...
  s(nodeID, k = 24, fx = FALSE, bs = 'cr', by = site) + site +
  sex + mean_fd", scalar)), data = df, method = "fREML", discrete = TRUE) 
...
fit_tractwise_bam <- function(tract_name, scalar) {
  ...
  s(age, k = 3, fx = T, bs = 'cr') + 
  ti(age, nodeID, k=c(3, 24), fx = F) + 
  s(nodeID, k = 24, fx = F, bs = 'cr', by = subjectID) + + s(subjectID, bs = 're') +
  s(nodeID, k = 24, fx = FALSE, bs = 'cr', by = site) + site +
  sex + mean_fd", scalar)), data = df, method = "fREML", discrete = TRUE) 
...
plot_compare_residuals <- function(tract, num_subjects, model_number) { 
  ...
  if(model_number == 1) {
    ...
    gam_df_toplot$residuals_bam_no_re <- resid(tractwise_bam_no_re[[tract]])
    subjects_to_plot <- subject_subset[c(1:num_subjects)]
    plot <- ggplot(gam_df_toplot[gam_df_toplot$subjectID %in% subjects_to_plot, ],
                       aes(x = nodeID, y = residuals_bam_no_re, color = factor(subjectID))) +
      geom_line() + theme(legend.position = "none",
                        plot.title = element_text(hjust=0.5, size = 12),
                        axis.title.x = element_blank(), 
                        axis.title.y = element_blank()) + 
...
saveRDS(tractwise_gam_re, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam_re_sites.RData")

tractwise_bam_no_re <- lapply(unique(gam_df$tract_hemi), fit_tractwise_bam_no_re, scalar="dti_md")
names(tractwise_bam_no_re) <- unique(gam_df$tract_hemi)
saveRDS(tractwise_bam_no_re, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_bam_no_re_sites.RData")
...
saveRDS(tractwise_bam, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_bam2_sites.RData")
...
tractwise_gam_re <- readRDS("/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam_re_sites.RData")
tractwise_bam <- readRDS("/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_bam2_sites.RData")

```

- 8/2/24: things to check
    + look at summary of original model, especially the ti() term to see if it's significant -- it is. but loses significance when we use the final model.
    + fit a new model where i keep nodeID, add site, and make new residual plots to include in my tractwise GAMs docs. 
    + fit a new model where i swap out nodeID with distance to cortex, add site
    + eventually: fit percent change for nodeID and distance to cortex models

```{r check model summary}


summary(tractwise_bam$Left_Anterior_Thalamic_Radiation)
summary(tractwise_bam$Left_Arcuate_Fasciculus)
summary(tractwise_bam$Right_Arcuate_Fasciculus)
summary(tractwise_bam$Left_Cingulum_Cingulate)
summary(tractwise_bam$Right_Cingulum_Cingulate)


summary(tractwise_bam$Left_Corticospinal_Tract)
summary(tractwise_bam$Right_Corticospinal_Tract)

summary(tractwise_bam_no_re$Left_Anterior_Thalamic_Radiation)
summary(tractwise_bam_no_re$Left_Arcuate_Fasciculus)
summary(tractwise_bam_no_re$Right_Arcuate_Fasciculus)
summary(tractwise_bam_no_re$Left_Cingulum_Cingulate)
summary(tractwise_bam_no_re$Right_Cingulum_Cingulate)
summary(tractwise_bam_no_re$Left_Corticospinal_Tract)
summary(tractwise_bam_no_re$Right_Corticospinal_Tract)

summary(tractwise_gam_re$Left_Anterior_Thalamic_Radiation)
summary(tractwise_gam_re$Left_Arcuate_Fasciculus)
summary(tractwise_gam_re$Right_Arcuate_Fasciculus)
```
...
plot_compare_acf <- function(tract_list, ncols = 5, model_number) {
  ...
  for (i in 1:num_plots) {
    ...
    if(model_number == 1) {
      ...
      acf(resid(tractwise_bam_no_re[[tract]], type = "response"), main = paste0(tract))
    } else if (model_number == 3) {

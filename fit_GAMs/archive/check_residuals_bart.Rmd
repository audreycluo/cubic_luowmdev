...
#tract_profiles <- fread("/cbica/projects/luo_wm_dev/input/HCPD/derivatives/HCPD_tractprofiles_dti/all_subjects/collated_tract_profiles_HCPD_reoriented.tsv")
...
subjects <- read.table("/cbica/projects/luo_wm_dev/input/HCPD/subject_list/HCPD_tempsubject_list_N572.txt")
...
#write.csv(gam_df, "/cbica/projects/luo_wm_dev/input/HCPD/derivatives/HCPD_tractprofiles_dti/all_subjects/gam_df_check_residuals.csv", row.names=F)
```


```{r}
demographics <- read.csv("/cbica/projects/luo_wm_dev/input/HCPD/sample_selection_files/HCPD_WMDev_TempSampleDemographics_N572_age8to22.csv")
site_data <- demographics %>% select(sub, site) %>% rename(subjectID=sub)
gam_df <- merge(gam_df, site_data, by = "subjectID")

write.csv(gam_df, "/cbica/projects/luo_wm_dev/input/HCPD/derivatives/HCPD_tractprofiles_dti/all_subjects/gam_df_check_residuals_site.csv", row.names=F)
```


```{r}
#gam_df <- read.csv("/cbica/projects/luo_wm_dev/input/HCPD/derivatives/HCPD_tractprofiles_dti/all_subjects/gam_df_check_residuals.csv")
gam_df <- read.csv("/cbica/projects/luo_wm_dev/input/HCPD/derivatives/HCPD_tractprofiles_dti/all_subjects/gam_df_check_residuals_site.csv")

```

```{r functions}
# fit tractwise gams
fit_tractwise_gam1 <- function(tract_name, scalar) {
  ...
  sex + mean_fd + s(subjectID, bs = 'fs', xt = list(bs='cr'))", scalar)), data = df,  method = "fREML", discrete = TRUE) 
...
fit_tractwise_gam2 <- function(tract_name, scalar) {
  df <- gam_df %>% filter(tract_hemi == tract_name)
  df$subjectID <- as.factor(df$subjectID)
  ...
  ti(age, nodeID, k=c(3, 24), fx = F) + 
  sex + mean_fd + 
  s(nodeID, subjectID, bs = 'sz', xt = list(bs='cr'))", scalar)), data = df) 
...
fit_tractwise_gam3 <- function(tract_name, scalar) {
  df <- gam_df %>% filter(tract_hemi == tract_name)
  df$subjectID <- as.factor(df$subjectID)
  te_gam <- gam(as.formula(sprintf("%s ~ 
  s(age, k = 3, fx = T, bs = 'cr') + 
  s(nodeID, subjectID, k = 24, fx = F, bs = 'cr', by = subjectID) + 
  ti(age, nodeID, k=c(3, 24), fx = F) + 
  + subjectID + sex + mean_fd", scalar)), data = df) 
...
# fit tractwise gams with site
fit_tractwise_gam1_site <- function(tract_name, scalar) {
  df <- gam_df %>% filter(tract_hemi == tract_name)
  df$subjectID <- as.factor(df$subjectID)
  ...
  s(nodeID, k = 24, fx = F, bs = 'cr') + 
  ti(age, nodeID, k=c(3, 24), fx = F) + 
  sex + mean_fd + s(subjectID, bs = 'fs', xt = list(bs='cr')) +
  s(nodeID,site, bs = 'sz', xt = list(bs='cr'))", scalar)), data = df) 
...
fit_tractwise_gam2_site <- function(tract_name, scalar) {
  df <- gam_df %>% filter(tract_hemi == tract_name)
  df$subjectID <- as.factor(df$subjectID)
  df$site <- as.factor(df$site)
  te_gam <- gam(as.formula(sprintf("%s ~ 
  ...
  ti(age, nodeID, k=c(3, 24), fx = F) + 
  sex + mean_fd + 
  s(nodeID, subjectID, bs = 'sz', xt = list(bs='cr')) +
  s(nodeID,site, bs = 'sz', xt = list(bs='cr'))", scalar)), data = df) 
...
fit_tractwise_gam3_site <- function(tract_name, scalar) {
  df <- gam_df %>% filter(tract_hemi == tract_name)
  df$subjectID <- as.factor(df$subjectID)
  df$site <- as.factor(df$site)
  te_gam <- gam(as.formula(sprintf("%s ~ 
...
plot_compare_residuals <- function(tract, num_subjects, model_number) { 
  ...
  if(model_number == 1) {
    ...
    gam_df_toplot$residuals_gam2 <- resid(tractwise_gam2[[tract]])
    subjects_to_plot <- subject_subset[c(1:num_subjects)]
    plot <- ggplot(gam_df_toplot[gam_df_toplot$subjectID %in% subjects_to_plot, ],
                       aes(x = nodeID, y = residuals_gam2, color = factor(subjectID))) +
      geom_line() + theme(legend.position = "none",
                        plot.title = element_text(hjust=0.5, size = 12),
                        axis.title.x = element_blank(), 
                        axis.title.y = element_blank()) + 
...
tractwise_gam1 <- lapply(unique(gam_df$tract_hemi), fit_tractwise_gam1, scalar="dti_md")
names(tractwise_gam1) <- unique(gam_df$tract_hemi)
saveRDS(tractwise_gam1, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam1_bart.RData")

tractwise_gam2 <- lapply(unique(gam_df$tract_hemi), fit_tractwise_gam2, scalar="dti_md")
names(tractwise_gam2) <- unique(gam_df$tract_hemi)
saveRDS(tractwise_gam2, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam2_bart.RData")
 

#Error in smooth.construct.cr.smooth.spec(object, dk$data, dk$knots) : 
  #Basis only handles 1D smooths
#tractwise_gam3 <- lapply(unique(gam_df$tract_hemi), fit_tractwise_gam3, scalar="dti_md")
#names(tractwise_gam3) <- unique(gam_df$tract_hemi)
#saveRDS(tractwise_gam3, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam3_bart.RData")

```

```{r fit gams site, eval=FALSE}
tractwise_gam1_site <- lapply(unique(gam_df$tract_hemi), fit_tractwise_gam1_site, scalar="dti_md")
names(tractwise_gam1_site) <- unique(gam_df$tract_hemi)
saveRDS(tractwise_gam1_site, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam1_site_bart.RData")

unique(gam_df$tract_hemi)
tractwise_gam2_site <- lapply(unique(gam_df$tract_hemi), fit_tractwise_gam2_site, scalar="dti_md")
names(tractwise_gam2_site) <- unique(gam_df$tract_hemi)
saveRDS(tractwise_gam2_site, "/Users/audluo/Desktop/check_resid_tractwise_gam2_site_bart.RData")
#saveRDS(tractwise_gam2_site, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam2_site_bart.RData")
 
tractwise_gam3_site <- lapply(unique(gam_df$tract_hemi), fit_tractwise_gam3_site, scalar="dti_md")
names(tractwise_gam3_site) <- unique(gam_df$tract_hemi)
saveRDS(tractwise_gam3_site, "/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam3_site_bart.RData")

``` 

```{r load gam models, cache = TRUE, cache.lazy = FALSE}

tractwise_gam1 <- readRDS("/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam1_bart.RData")
tractwise_gam2 <- readRDS("/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam2_bart.RData")
tractwise_gam3 <- readRDS("/cbica/projects/luo_wm_dev/output/HCPD/tract_profiles/GAM/dti_md/check_resid_tractwise_gam3_bart.RData")

```


I fit 3 tractwise models on a subset of HCP-D (full sample = 569; subsetted sample = 75 that represents the full age range)
spoiler: Model 3 is the best.

1) Model 1: include a simple nodeID smooth and subjectID as a random intercept (most similar to GAM used in Tractable)
`gam(dti_md ~ s(age, k = 3, fx = T, bs = 'cr') + s(nodeID, k = 24, fx = F, bs = 'cr') + ti(age, nodeID, k=c(3, 24), fx = F) + sex + mean_fd + s(subjectID, bs = 'fs', xt = list(bs='cr')), data = df)`

2) Model 2: include `by = subjectID` in the nodeID smooth but NO random intercept for subject ID
`gam(dti_md ~ s(age, k = 3, fx = T, bs = 'cr') + s(nodeID, k = 24, fx = F, bs = 'cr') + ti(age, nodeID, k=c(3, 24), fx = F) + sex + mean_fd + s(nodeID, subjectID, bs = 'sz', xt = list(bs='cr')))`

 
3) Model 3: include `by = subjectID` in the nodeID smooth and subjectID as a random intercept 
...
plot_15_subs_final <- arrange_tracts_hemis(plot_15_subs, "Raw Residual Curves from Model 1: \n s(age, k = 3, fx = T, bs = 'cr') + s(nodeID, k = 24, fx = F, bs = 'cr') + \nti(age, nodeID, k=c(3, 24), fx = F) + \nsex + mean_fd + s(subjectID, bs = 'fs', xt = list(bs='cr'))")
grid.arrange(arrangeGrob(plot_15_subs_final, left = y.grob, bottom = x.grob, right = space.grob))
ggsave("/Users/audluo/Desktop/model1.png", grid.arrange(arrangeGrob(plot_15_subs_final, left = y.grob, bottom = x.grob, right = space.grob)), width=17, height=20, unit = "in")
```
...
plot_15_subs_final <- arrange_tracts_hemis(plot_15_subs, "Smooth Residual Fit from Model 2: \n s(age, k = 3, fx = T, bs = 'cr') + s(nodeID, k = 24, fx = F, bs = 'cr') + \n ti(age, nodeID, k=c(3, 24), fx = F) + sex + mean_fd + \ns(nodeID, subjectID, bs = 'sz', xt = list(bs='cr')))")
grid.arrange(arrangeGrob(plot_15_subs_final, left = y.grob, bottom = x.grob, right = space.grob))

ggsave("/Users/audluo/Desktop/model2.png", grid.arrange(arrangeGrob(plot_15_subs_final, left = y.grob, bottom = x.grob, right = space.grob)), width=17, height=20, unit = "in")
```

# Residual plots for Model 3 for each tract (showing 15 subjects only)
- seeing flat squiggles centered around zero - good!
- from Jeff: "Only thing I notice is that the boundaries often have sharp oscillations — but that’s a pretty minor issue due to how the smoother is implemented, and definitely not an easy fix. I’d say if y’all’re happy with these fits they should work." 
```{r residual plot model 3, fig.width=17, fig.height=20}
tracts <- unique(gam_df$tract_hemi)
...
plot_15_subs <- lapply(tracts, plot_compare_residuals, 15, model_number = 3)
names(plot_15_subs) <- tracts
 

plot_15_subs_final <- arrange_tracts_hemis(plot_15_subs, "Smooth Residual Fit from Model 3: \n s(age, k = 3, fx = T, bs = 'cr') + \ns(nodeID, k = 24, fx = F, bs = 'cr', by = subjectID) + \nti(age, nodeID, k=c(3, 24), fx = F) + \ns(subjectID, bs = 're') + sex + mean_fd")
grid.arrange(arrangeGrob(plot_15_subs_final, left = y.grob, bottom = x.grob, right = space.grob))
```
...
plot_compare_acf <- function(tract_list, ncols = 5, model_number) {
  ...
  for (i in 1:num_plots) {
    tract <- tract_list[i]
    if(model_number == 1) {
      acf(resid(tractwise_gam1[[tract]], type = "response"), main = paste0(tract))
    } else if (model_number == 2) {
      acf(resid(tractwise_gam2[[tract]], type = "response"), main = paste0(tract))
    } else if (model_number == 3) {
...
```{r model 3 acf, fig.height=15, fig.width=15}
plot_compare_acf(unique(gam_df$tract_hemi), model_number = 3)
```

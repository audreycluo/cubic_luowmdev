...
r_script="/cbica/projects/luo_wm_dev/code/tract_profiles/fit_GAMs/fit_GAMs_development_combat_all.R"
...
for dataset in "${datasets[@]}"; do
  ...
  for scalar in "${scalars[@]}"; do
      ...
      sbatch --job-name=${job_name} --nodes=1 --ntasks=1 --cpus-per-task=4 --mem=8G --time=6:00:00 --output=${logs_dir}/${job_name}_combat_all_%j.out --error=${logs_dir}/${job_name}_combat_all_%j.err --wrap="singularity run --cleanenv /cbica/projects/luo_wm_dev/software/docker/r-packages-for-cubic_0.0.5.sif Rscript --save ${r_script} ${dataset} ${scalar}"

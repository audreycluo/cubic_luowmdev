...
PNC_deveffects_5 <- average_tract_deveffect(ageeffect.fdr_dfs$PNC_ageeffects, ageeffect_measure, bin_num_nodes = 5, clipEnds = 3)  
PNC_deveffects_10 <- average_tract_deveffect(ageeffect.fdr_dfs$PNC_ageeffects, ageeffect_measure, bin_num_nodes = 10, clipEnds = 3)  
...
```{r assign age effects to cortical endpoints PNC}
...
CSTL_deveffect_PNC <- lh_maps_PNC$LeftCorticospinal %>% mutate(
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      PNC_deveffects_5 %>% filter(tractID == "Left_Corticospinal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSTR_deveffect_PNC <- rh_maps_PNC$RightCorticospinal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Right_Corticospinal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
ARCL_deveffect_PNC <- lh_maps_PNC$LeftArcuate %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region, cortex) %>%
  ...
  mutate(
    mean_age_effect = case_when(
      !is.na(thresh_probability) & (str_detect(cortex, "(?i)frontal") | str_detect(cortex, "(?i)motor") | str_detect(cortex, "(?i)Posterior_Opercular")) ~ 
        PNC_deveffects_5 %>% filter(tractID == "Left_Arcuate", node_position == "end1") %>% pull(mean_ageeffect),
      !is.na(thresh_probability) & (str_detect(cortex, "(?i)Temp") | str_detect(cortex, "(?i)Auditory")) ~ 
        PNC_deveffects_5 %>% filter(tractID == "Left_Arcuate", node_position == "end2") %>% pull(mean_ageeffect),
      TRUE ~ NA_real_
    )
  ) %>%
  mutate(
...
ARCR_deveffect_PNC <- rh_maps_PNC$RightArcuate %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
...
ILFL_deveffect_PNC <- lh_maps_PNC$LeftInferiorLongitudinal %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = case_when(
      ...
      !str_detect(cortex, "(?i)visual|Inferior_Parietal|Occipital") ~ 
        PNC_deveffects_5 %>% filter(tractID == "Left_Inferior_Longitudinal", node_position == "end1") %>% pull(mean_ageeffect),
      !is.na(thresh_probability) & 
      str_detect(cortex, "(?i)visual|Inferior_Parietal|Occipital") ~ 
        PNC_deveffects_5 %>% filter(tractID == "Left_Inferior_Longitudinal", node_position == "end2") %>% pull(mean_ageeffect),
      TRUE ~ NA_real_
    ),
    end = case_when(
      !is.na(thresh_probability) & 
...
ILFR_deveffect_PNC <- rh_maps_PNC$RightInferiorLongitudinal %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      PNC_deveffects_5 %>%  filter(tractID == "Right_Inferior_Longitudinal", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & (str_detect(cortex, "(?i)visual")) | region == "PHA3", 
      PNC_deveffects_5 %>%  filter(tractID == "Right_Inferior_Longitudinal", node_position == "end2") %>% pull(mean_ageeffect),
        NA))
    ) %>%
  mutate(
    end = ifelse(
...
IFOL_deveffect_PNC <- lh_maps_PNC$LeftInferiorFrontooccipital %>%
  mutate(
  ...
  mutate(mean_age_effect = case_when(
    ...
    (str_detect(cortex, "(?i)frontal|Auditory")) ~ 
      PNC_deveffects_5 %>% filter(tractID == "Left_Inferior_Fronto.occipital", node_position == "end1") %>% pull(mean_ageeffect),
    !is.na(thresh_probability) & 
    str_detect(cortex, "(?i)visual") ~ 
      PNC_deveffects_5 %>% filter(tractID == "Left_Inferior_Fronto.occipital", node_position == "end2") %>% pull(mean_ageeffect),
    TRUE ~ NA_real_)) %>%
  mutate(
    end = ifelse(
      !is.na(thresh_probability) & 
...
IFOR_deveffect_PNC <- rh_maps_PNC$RightInferiorFrontooccipital %>%
  mutate(
    thresh_probability = ifelse(probability < 0.1, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      (str_detect(cortex, "(?i)frontal")  |
       str_detect(cortex, "Auditory")), 
      PNC_deveffects_5 %>% filter(tractID == "Right_Inferior_Fronto.occipital", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)visual")), 
       PNC_deveffects_5 %>% filter(tractID == "Right_Inferior_Fronto.occipital", node_position == "end2") %>% pull(mean_ageeffect),
        NA
      )
    )
  ) %>%
...
SLFL_deveffect_PNC <- lh_maps_PNC$LeftSuperiorLongitudinal %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      PNC_deveffects_5 %>% filter(tractID == "Left_Superior_Longitudinal", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)pariet|(?i)auditory")), 
      PNC_deveffects_5 %>% filter(tractID == "Left_Superior_Longitudinal", node_position == "end2") %>% pull(mean_ageeffect),
        NA))) %>%
  mutate(end = ifelse(
      !is.na(thresh_probability) & 
      (str_detect(cortex, "(?i)frontal") |
...
SLFR_deveffect_PNC <- rh_maps_PNC$RightSuperiorLongitudinal %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      (str_detect(cortex, "(?i)frontal") |
         str_detect(cortex, "(?i)motor") | 
         str_detect(cortex, "(?i)Opercular")), 
      PNC_deveffects_5 %>% filter(tractID == "Right_Superior_Longitudinal", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)pariet")), 
      PNC_deveffects_5 %>% filter(tractID == "Right_Superior_Longitudinal", node_position == "end2") %>% pull(mean_ageeffect),
      NA))) %>%
  mutate(
    end = ifelse(
      !is.na(thresh_probability) & 
...
pARCL_deveffect_PNC <- lh_maps_PNC$LeftPosteriorArcuate %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse( 
      ...
      PNC_deveffects_5 %>% filter(tractID == "Left_Posterior_Arcuate", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)temporal") |
           str_detect(cortex, "(?i)visual") |
           str_detect(cortex, "(?i)auditory")), 
       PNC_deveffects_5 %>% filter(tractID == "Left_Posterior_Arcuate", node_position == "end2") %>% pull(mean_ageeffect), 
        NA))) %>%
  mutate(
    end = ifelse( 
      !is.na(thresh_probability) & 
...
pARCR_deveffect_PNC <- rh_maps_PNC$RightPosteriorArcuate %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse( 
      !is.na(thresh_probability) & 
      (str_detect(cortex, "(?i)parietal")),  # might exclude TPOJ1 since its between the two endpoints
      PNC_deveffects_5 %>% filter(tractID == "Right_Posterior_Arcuate", node_position == "end1") %>% pull(mean_ageeffect), 
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)temporal") |
           str_detect(cortex, "(?i)visual") |
           str_detect(cortex, "(?i)auditory")), 
        PNC_deveffects_5 %>% filter(tractID == "Right_Posterior_Arcuate", node_position == "end2") %>% pull(mean_ageeffect), 
        NA
      )
    )
  ) %>%
...
VOFL_deveffect_PNC <- lh_maps_PNC$LeftVerticalOccipital %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
    ...
    mean_age_effect = ifelse( 
      ...
      (str_detect(cortex, "(?i)dorsal") | 
       str_detect(cortex, "(?i)parietal") |
         ...
         region == "TPOJ3" |
         region == "LO3"),   
      PNC_deveffects_5 %>% filter(tractID == "Left_Vertical_Occipital", node_position == "end1") %>% pull(mean_ageeffect), 
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)ventral") | 
         ...
         region == "LO2" |
           region == "V3"), 
        PNC_deveffects_5 %>% filter(tractID == "Left_Vertical_Occipital", node_position == "end2") %>% pull(mean_ageeffect), 
        NA))) %>%
  mutate(
    end = ifelse( 
      !is.na(thresh_probability) & 
...
VOFR_deveffect_PNC <- rh_maps_PNC$RightVerticalOccipital %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse( 
      ...
      (str_detect(cortex, "(?i)dorsal") | 
       ...
       region == "V4" | # kind of arbitrarily placing V3 and V4 for now
       region == "LO3"),   
       PNC_deveffects_5 %>% filter(tractID == "Right_Vertical_Occipital", node_position == "end1") %>% pull(mean_ageeffect), 
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)ventral") | 
         region == "PH" |
         ...
         region == "MST" |
         region == "V3"), 
         PNC_deveffects_5 %>% filter(tractID == "Right_Vertical_Occipital", node_position == "end2") %>% pull(mean_ageeffect), NA
      )
    )
  ) %>%
  mutate(
...
# callosum bundles
COrbL_deveffect_PNC <- lh_maps_PNC$LeftCallosumOrbital %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Orbital" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
COrbR_deveffect_PNC <- rh_maps_PNC$RightCallosumOrbital %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Orbital" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CAntFrL_deveffect_PNC <- lh_maps_PNC$LeftCallosumAnteriorFrontal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Anterior_Frontal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CAntFrR_deveffect_PNC <- rh_maps_PNC$RightCallosumAnteriorFrontal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Anterior_Frontal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSupFrL_deveffect_PNC <- lh_maps_PNC$LeftCallosumSuperiorFrontal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Superior_Frontal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSupFrR_deveffect_PNC <- rh_maps_PNC$RightCallosumSuperiorFrontal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Superior_Frontal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CMotL_deveffect_PNC <- lh_maps_PNC$LeftCallosumMotor %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Motor" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CMotR_deveffect_PNC <- rh_maps_PNC$RightCallosumMotor %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Motor" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSupParL_deveffect_PNC <- lh_maps_PNC$LeftCallosumSuperiorParietal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Superior_Parietal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSupParR_deveffect_PNC <- rh_maps_PNC$RightCallosumSuperiorParietal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Superior_Parietal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CPostParL_deveffect_PNC <- lh_maps_PNC$LeftCallosumPosteriorParietal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Posterior_Parietal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CPostParR_deveffect_PNC <- rh_maps_PNC$RightCallosumPosteriorParietal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Posterior_Parietal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CTempL_deveffect_PNC <- lh_maps_PNC$LeftCallosumTemporal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Temporal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CTempR_deveffect_PNC <- rh_maps_PNC$RightCallosumTemporal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Temporal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
COccL_deveffect_PNC <- lh_maps_PNC$LeftCallosumOccipital %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Occipital" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
COccR_deveffect_PNC <- rh_maps_PNC$RightCallosumOccipital %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      PNC_deveffects_5 %>% filter(tractID == "Callosum_Occipital" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
arrange_ageeffect_plots <- function(lh_plots, rh_plots) {
  ...
  # save out
  #ggsave(sprintf("/Users/audluo/PennLINC/luowm_local/output/tract_profiles_testing/PNC/tract_to_region_ageeffects.png"),
...
lh_ageeffect_plots <- lapply(c("ARCL", "CSTL" ,"ILFL", "IFOL", "SLFL", "pARCL", "VOFL"), plot_ageeffects, dataset="PNC", ylim1 = 0.15, ylim2 = 0.36)
rh_ageeffect_plots <- lapply(c("ARCR", "CSTR", "ILFR", "IFOR", "SLFR", "pARCR", "VOFR"), plot_ageeffects, dataset="PNC", ylim1 = 0.15, ylim2 = 0.36)
...
ggsave(paste0(png_dir, "/", "PNC", "/","dev.png"), age_effect_plots, height = 14, width = 14, units = "in")
...
```{r callosum PNC age effects, fig.height = 20, fig.width = 15}
# callosum bundles
# plot age effects for each tract
lh_ageeffect_callosum_plots <- lapply(c("COrbL", "CAntFrL" ,"CSupFrL", "CMotL", "CSupParL", "CPostParL", "CTempL", "COccL"), plot_ageeffects, dataset="PNC", ylim1 = 0.15, ylim2 = 0.36)
rh_ageeffect_callosum_plots <- lapply(c("COrbR", "CAntFrR" ,"CSupFrR", "CMotR", "CSupParR", "CPostParR", "CTempR", "COccR"), plot_ageeffects, dataset="PNC", ylim1 = 0.15, ylim2 = 0.36)
...
ggsave(paste0(png_dir, "/", "PNC", "/","dev_callosum.png"), age_effect_callosum_plots_PNC, height = 20, width = 15, units = "in")
...
lh_by_region <- rbind(COrbL_deveffect_PNC, CAntFrL_deveffect_PNC, 
                      CSupFrL_deveffect_PNC, CMotL_deveffect_PNC, 
                      CSupParL_deveffect_PNC, CPostParL_deveffect_PNC, 
                      CTempL_deveffect_PNC, COccL_deveffect_PNC, 
                      ARCL_deveffect_PNC, IFOL_deveffect_PNC, SLFL_deveffect_PNC) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))

length(which(is.na(lh_by_region$regional_mean_ageeffect)))  

# lh all
lh_by_region_all <- rbind(COrbL_deveffect_PNC, CAntFrL_deveffect_PNC, 
                      CSupFrL_deveffect_PNC, CMotL_deveffect_PNC, 
                      CSupParL_deveffect_PNC, CPostParL_deveffect_PNC, 
                      CTempL_deveffect_PNC, COccL_deveffect_PNC, 
                      ARCL_deveffect_PNC, IFOL_deveffect_PNC, SLFL_deveffect_PNC,
                      CSTL_deveffect_PNC, ILFL_deveffect_PNC, pARCL_deveffect_PNC, VOFL_deveffect_PNC) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))

length(which(is.na(lh_by_region_all$regional_mean_ageeffect)))  

# lh callosum + most tracts minus pARC and VOF
lh_by_region_subset <- rbind(COrbL_deveffect_PNC, CAntFrL_deveffect_PNC, 
                      CSupFrL_deveffect_PNC, CMotL_deveffect_PNC, 
                      CSupParL_deveffect_PNC, CPostParL_deveffect_PNC, 
                      CTempL_deveffect_PNC, COccL_deveffect_PNC, 
                      ARCL_deveffect_PNC, IFOL_deveffect_PNC, SLFL_deveffect_PNC,
                      CSTL_deveffect_PNC, ILFL_deveffect_PNC) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))

length(which(is.na(lh_by_region_subset$regional_mean_ageeffect)))  

# callosum + tracts of interest only
rh_by_region <- rbind(COrbR_deveffect_PNC, CAntFrR_deveffect_PNC, 
                      CSupFrR_deveffect_PNC, CMotR_deveffect_PNC, 
                      CSupParR_deveffect_PNC, CPostParR_deveffect_PNC, 
                      CTempR_deveffect_PNC, COccR_deveffect_PNC, 
                      ARCR_deveffect_PNC, IFOR_deveffect_PNC, SLFR_deveffect_PNC) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))

length(which(is.na(rh_by_region$regional_mean_ageeffect))) 


rh_by_region_all <- rbind(COrbR_deveffect_PNC, CAntFrR_deveffect_PNC, 
                      CSupFrR_deveffect_PNC, CMotR_deveffect_PNC, 
                      CSupParR_deveffect_PNC, CPostParR_deveffect_PNC, 
                      CTempR_deveffect_PNC, COccR_deveffect_PNC, 
                      ARCR_deveffect_PNC, IFOR_deveffect_PNC, SLFR_deveffect_PNC,
                      CSTR_deveffect_PNC, ILFR_deveffect_PNC, pARCR_deveffect_PNC, VOFR_deveffect_PNC) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))
...
rh_by_region_subset <- rbind(COrbL_deveffect_PNC, CAntFrL_deveffect_PNC, 
                      CSupFrL_deveffect_PNC, CMotL_deveffect_PNC, 
                      CSupParL_deveffect_PNC, CPostParL_deveffect_PNC, 
                      CTempL_deveffect_PNC, COccL_deveffect_PNC, 
                      ARCL_deveffect_PNC, IFOL_deveffect_PNC, SLFL_deveffect_PNC,
...
#write.csv(lh_by_region, "/cbica/projects/luo_wm_dev/output/PNC/tract_profiles/tract_to_cortex/PNC_lh_agg.csv", row.names=F)
#write.csv(rh_by_region, "/cbica/projects/luo_wm_dev/output/PNC/tract_profiles/tract_to_cortex/PNC_rh_agg.csv", row.names=F)
```
...
ggsave(paste0(png_dir, "/", "PNC", "/","dev_aggregated.png"), agg_tracts_interest , height = 10, width = 10, units = "in")

agg_all <- plot_aggregated_maps(lh_by_region_all, rh_by_region_all)
ggsave(paste0(png_dir, "/", "PNC", "/","dev_all.png"), agg_all , height = 10, width = 10, units = "in")

agg_subset <- plot_aggregated_maps(lh_by_region_subset, rh_by_region_subset)
ggsave(paste0(png_dir, "/", "PNC", "/","dev_aggregated_subset.png"), agg_subset, height = 10, width = 10, units = "in")
...
lh_SAranks <- lapply(lh_names, merge_SAaxis, "all_regions", dataset="PNC")
lh_SAranks_endpoints <- lapply(lh_names, merge_SAaxis, "endpoints_only", dataset="PNC")
 
rh_names <- c("ARCR", "CSTR" ,"ILFR", "IFOR", "SLFR", "pARCR", "VOFR", "COrbR", "CAntFrR" ,"CSupFrR", "CMotR", "CSupParR", "CPostParR", "CTempR", "COccR")
rh_SAranks <- lapply(rh_names, merge_SAaxis, "all_regions", dataset="PNC")
rh_SAranks_endpoints <- lapply(rh_names, merge_SAaxis, "endpoints_only", dataset="PNC")
...
cor.test(all_endpoints$mean_SA, all_endpoints$age_effect)
...
cor.test(abs_diffs_all$mean_SA_diff, abs_diffs_all$age_effect_diff) # there is an effect here for PNC but probably wouldn't survive spinning?

ggsave(ggsave(paste0(png_dir, "/", "PNC", "/","delta_delta.png"), diffs_plot, height = 6, width = 9, units = "in")


ggsave(ggsave(paste0(png_dir, "/", "PNC", "/","delta_delta_abs.png"), abs_diff_plot, height = 6, width = 9, units = "in")
...
PNC_agg_ageeffects <- agg_final

 
write.csv(PNC_agg_ageeffects, "/cbica/projects/luo_wm_dev/output/PNC/tract_profiles/tract_to_cortex/PNC_agg_ageeffects_ordered_all.csv")
cor.test(t1t2_ageeffect_HCPD$data, PNC_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = 0.15, p = 0.0004 spearman (all)
 

cor.test(t1t2_mean_HCPD$data, PNC_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = 0.14, p = 0.03 spearman (all)
cor.test(t1t2_roc_HCPD$data, PNC_agg_ageeffects$regional_mean_ageeffect, method = 'spearman')  

cor.test(t1t2_maxslopeage_HCPD$data, PNC_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # meh

cor.test(alff_pnc$data, PNC_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = -0.04, p =NS
cor.test(alff_decline_pnc$data, PNC_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = 0.26, pval = 0.00005

 
 
hist(t1t2_ageeffect_PNC$data)
...
perm.sphere.p(x = PNC_agg_ageeffects$regional_mean_ageeffect, y = t1t2_ageeffect_HCPD$data, perm.id = perm.id.full, corr.type = "spearman")    
perm.sphere.p(x = PNC_agg_ageeffects$regional_mean_ageeffect, y = t1t2_mean_HCPD$data, perm.id = perm.id.full, corr.type = "spearman")  
perm.sphere.p(x = PNC_agg_ageeffects$regional_mean_ageeffect, y = t1t2_roc_HCPD$data, perm.id = perm.id.full, corr.type = "spearman")   
perm.sphere.p(x = PNC_agg_ageeffects$regional_mean_ageeffect, y = t1t2_maxslopeage_HCPD$data, perm.id = perm.id.full, corr.type = "spearman")   

perm.sphere.p(x = PNC_agg_ageeffects$regional_mean_ageeffect, y = alff_pnc$data, perm.id = perm.id.full, corr.type = "spearman")   
perm.sphere.p(x = PNC_agg_ageeffects$regional_mean_ageeffect, y = alff_decline_pnc$data, perm.id = perm.id.full, corr.type = "spearman")   
...
cor.test(PNC_agg$regional_mean_ageeffect, HCPD_agg$regional_mean_ageeffect, method = "pearson")



cor.test(PNC_agg$regional_mean_ageeffect, PNC_agg_ageeffects$regional_mean_ageeffect, method = "pearson") # bin 10 and bin 5 are the same pretty much

perm.sphere.p(x = PNC_agg$regional_mean_ageeffect, y = HCPD_agg$regional_mean_ageeffect, perm.id = perm.id.full, corr.type = "pearson")  # r = 0.8, p < 0.0001
```

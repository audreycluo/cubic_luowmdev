...
HCPD_deveffects_5 <- average_tract_deveffect(ageeffect.fdr_dfs$HCPD_ageeffects, ageeffect_measure, bin_num_nodes = 5, clipEnds = 3)  
HCPD_deveffects_10 <- average_tract_deveffect(ageeffect.fdr_dfs$HCPD_ageeffects, ageeffect_measure, bin_num_nodes = 10, clipEnds = 3)  
...
```{r assign age effects to cortical endpoints HCPD}
...
CSTL_deveffect_HCPD <- lh_maps_HCPD$LeftCorticospinal %>% mutate(
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      HCPD_deveffects_5 %>% filter(tractID == "Left_Corticospinal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSTR_deveffect_HCPD <- rh_maps_HCPD$RightCorticospinal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Right_Corticospinal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
ARCL_deveffect_HCPD <- lh_maps_HCPD$LeftArcuate %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region, cortex) %>%
  ...
  mutate(
    mean_age_effect = case_when(
      !is.na(thresh_probability) & (str_detect(cortex, "(?i)frontal") | str_detect(cortex, "(?i)motor") | str_detect(cortex, "(?i)Posterior_Opercular")) ~ 
        HCPD_deveffects_5 %>% filter(tractID == "Left_Arcuate", node_position == "end1") %>% pull(mean_ageeffect),
      !is.na(thresh_probability) & (str_detect(cortex, "(?i)Temp") | str_detect(cortex, "(?i)Auditory")) ~ 
        HCPD_deveffects_5 %>% filter(tractID == "Left_Arcuate", node_position == "end2") %>% pull(mean_ageeffect),
      TRUE ~ NA_real_
    )
  ) %>%
  mutate(
...
ARCR_deveffect_HCPD <- rh_maps_HCPD$RightArcuate %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
...
ILFL_deveffect_HCPD <- lh_maps_HCPD$LeftInferiorLongitudinal %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = case_when(
      ...
      !str_detect(cortex, "(?i)visual|Inferior_Parietal|Occipital") ~ 
        HCPD_deveffects_5 %>% filter(tractID == "Left_Inferior_Longitudinal", node_position == "end1") %>% pull(mean_ageeffect),
      !is.na(thresh_probability) & 
      str_detect(cortex, "(?i)visual|Inferior_Parietal|Occipital") ~ 
        HCPD_deveffects_5 %>% filter(tractID == "Left_Inferior_Longitudinal", node_position == "end2") %>% pull(mean_ageeffect),
      TRUE ~ NA_real_
    ),
    end = case_when(
      !is.na(thresh_probability) & 
...
ILFR_deveffect_HCPD <- rh_maps_HCPD$RightInferiorLongitudinal %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      HCPD_deveffects_5 %>%  filter(tractID == "Right_Inferior_Longitudinal", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & (str_detect(cortex, "(?i)visual") ), 
      HCPD_deveffects_5 %>%  filter(tractID == "Right_Inferior_Longitudinal", node_position == "end2") %>% pull(mean_ageeffect),
        NA))
    ) %>%
  mutate(
    end = ifelse(
...
IFOL_deveffect_HCPD <- lh_maps_HCPD$LeftInferiorFrontooccipital %>%
  mutate(
  ...
  mutate(mean_age_effect = case_when(
    ...
    (str_detect(cortex, "(?i)frontal|Auditory")) ~ 
      HCPD_deveffects_5 %>% filter(tractID == "Left_Inferior_Fronto.occipital", node_position == "end1") %>% pull(mean_ageeffect),
    !is.na(thresh_probability) & 
    str_detect(cortex, "(?i)visual") ~ 
      HCPD_deveffects_5 %>% filter(tractID == "Left_Inferior_Fronto.occipital", node_position == "end2") %>% pull(mean_ageeffect),
    TRUE ~ NA_real_)) %>%
  mutate(
    end = ifelse(
      !is.na(thresh_probability) & 
...
IFOR_deveffect_HCPD <- rh_maps_HCPD$RightInferiorFrontooccipital %>%
  mutate(
    thresh_probability = ifelse(probability < 0.1, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      (str_detect(cortex, "(?i)frontal")  |
       str_detect(cortex, "Auditory")), 
      HCPD_deveffects_5 %>% filter(tractID == "Right_Inferior_Fronto.occipital", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)visual")), 
       HCPD_deveffects_5 %>% filter(tractID == "Right_Inferior_Fronto.occipital", node_position == "end2") %>% pull(mean_ageeffect),
        NA
      )
    )
  ) %>%
...
SLFL_deveffect_HCPD <- lh_maps_HCPD$LeftSuperiorLongitudinal %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      HCPD_deveffects_5 %>% filter(tractID == "Left_Superior_Longitudinal", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)pariet")), 
      HCPD_deveffects_5 %>% filter(tractID == "Left_Superior_Longitudinal", node_position == "end2") %>% pull(mean_ageeffect),
        NA))) %>%
  mutate(end = ifelse(
      !is.na(thresh_probability) & 
      (str_detect(cortex, "(?i)frontal") |
...
SLFR_deveffect_HCPD <- rh_maps_HCPD$RightSuperiorLongitudinal %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse(
      ...
      (str_detect(cortex, "(?i)frontal") |
         str_detect(cortex, "motor") | 
         str_detect(cortex, "(?i)Opercular")), 
      HCPD_deveffects_5 %>% filter(tractID == "Right_Superior_Longitudinal", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)pariet")), 
      HCPD_deveffects_5 %>% filter(tractID == "Right_Superior_Longitudinal", node_position == "end2") %>% pull(mean_ageeffect),
      NA))) %>%
  mutate(
    end = ifelse(
      !is.na(thresh_probability) & 
...
pARCL_deveffect_HCPD <- lh_maps_HCPD$LeftPosteriorArcuate %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse( 
      ...
      HCPD_deveffects_5 %>% filter(tractID == "Left_Posterior_Arcuate", node_position == "end1") %>% pull(mean_ageeffect),
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)temporal") |
           str_detect(cortex, "(?i)visual") |
           str_detect(cortex, "(?i)auditory")), 
       HCPD_deveffects_5 %>% filter(tractID == "Left_Posterior_Arcuate", node_position == "end2") %>% pull(mean_ageeffect), 
        NA))) %>%
  mutate(
    end = ifelse( 
      !is.na(thresh_probability) & 
...
pARCR_deveffect_HCPD <- rh_maps_HCPD$RightPosteriorArcuate %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse( 
      !is.na(thresh_probability) & 
      (str_detect(cortex, "(?i)parietal")),  # might exclude TPOJ1 since its between the two endpoints
      HCPD_deveffects_5 %>% filter(tractID == "Right_Posterior_Arcuate", node_position == "end1") %>% pull(mean_ageeffect), 
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)temporal") |
           str_detect(cortex, "(?i)visual") |
           str_detect(cortex, "(?i)auditory")), 
        HCPD_deveffects_5 %>% filter(tractID == "Right_Posterior_Arcuate", node_position == "end2") %>% pull(mean_ageeffect), 
        NA
      )
    )
  ) %>%
...
VOFL_deveffect_HCPD <- lh_maps_HCPD$LeftVerticalOccipital %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse( 
      ...
      HCPD_deveffects_5 %>% filter(tractID == "Left_Vertical_Occipital", node_position == "end1") %>% pull(mean_ageeffect), 
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)ventral") | 
         ...
         region == "LO2" |
           region == "V3"), 
        HCPD_deveffects_5 %>% filter(tractID == "Left_Vertical_Occipital", node_position == "end2") %>% pull(mean_ageeffect), 
        NA
      )
    )
  ) %>%
...
VOFR_deveffect_HCPD <- rh_maps_HCPD$RightVerticalOccipital %>%
  mutate(
    thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>%
  select(thresh_probability, regionName, region,  cortex) %>%
  ...
  mutate(
    mean_age_effect = ifelse( 
      ...
      (str_detect(cortex, "(?i)dorsal") | 
       ...
       HCPD_deveffects_5 %>% filter(tractID == "Right_Vertical_Occipital", node_position == "end1") %>% pull(mean_ageeffect), 
      ifelse(
        !is.na(thresh_probability) & 
        (str_detect(cortex, "(?i)ventral") | 
         region == "PH" |
         region == "LO2" |
           region == "V3"), 
         HCPD_deveffects_5 %>% filter(tractID == "Right_Vertical_Occipital", node_position == "end2") %>% pull(mean_ageeffect), NA
      )
    )
  ) %>%
  mutate(
...
# callosum bundles
COrbL_deveffect_HCPD <- lh_maps_HCPD$LeftCallosumOrbital %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Orbital" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
COrbR_deveffect_HCPD <- rh_maps_HCPD$RightCallosumOrbital %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Orbital" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CAntFrL_deveffect_HCPD <- lh_maps_HCPD$LeftCallosumAnteriorFrontal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Anterior_Frontal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CAntFrR_deveffect_HCPD <- rh_maps_HCPD$RightCallosumAnteriorFrontal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Anterior_Frontal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSupFrL_deveffect_HCPD <- lh_maps_HCPD$LeftCallosumSuperiorFrontal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Superior_Frontal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSupFrR_deveffect_HCPD <- rh_maps_HCPD$RightCallosumSuperiorFrontal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Superior_Frontal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CMotL_deveffect_HCPD <- lh_maps_HCPD$LeftCallosumMotor %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Motor" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CMotR_deveffect_HCPD <- rh_maps_HCPD$RightCallosumMotor %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Motor" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSupParL_deveffect_HCPD <- lh_maps_HCPD$LeftCallosumSuperiorParietal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Superior_Parietal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CSupParR_deveffect_HCPD <- rh_maps_HCPD$RightCallosumSuperiorParietal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Superior_Parietal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CPostParL_deveffect_HCPD <- lh_maps_HCPD$LeftCallosumPosteriorParietal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Posterior_Parietal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CPostParR_deveffect_HCPD <- rh_maps_HCPD$RightCallosumPosteriorParietal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Posterior_Parietal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CTempL_deveffect_HCPD <- lh_maps_HCPD$LeftCallosumTemporal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Temporal" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
CTempR_deveffect_HCPD <- rh_maps_HCPD$RightCallosumTemporal %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Temporal" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
COccL_deveffect_HCPD <- lh_maps_HCPD$LeftCallosumOccipital %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Occipital" & node_position == "end2") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
COccR_deveffect_HCPD <- rh_maps_HCPD$RightCallosumOccipital %>% mutate(
  thresh_probability = ifelse(probability < threshold, NA, probability)
  ) %>% 
  select(thresh_probability, regionName, region, cortex) %>%
  arrange(thresh_probability) %>% 
  mutate(
    mean_age_effect = ifelse(
      !is.na(thresh_probability), 
      HCPD_deveffects_5 %>% filter(tractID == "Callosum_Occipital" & node_position == "end1") %>% pull(mean_ageeffect), NA)
    ) %>% 
   mutate(
    end = ifelse(
      !is.na(thresh_probability), 
...
plot_ageeffects <- function(bundle_name, dataset) {
  df <- get(paste0(bundle_name, "_deveffect_", dataset))
  if(grepl("L$", bundle_name)) {
    hemi = "left"
    cortical_pos <- c("left lateral", "left medial")
  ...
  plot <- ggplot() + 
      geom_brain(data = df, atlas= glasser, 
                 ...
                 hemi = hemi,
                 position = position_brain(cortical_pos)) + 
    scale_fill_gradientn(colors = aquamarine, na.value = "grey92", limits = c(0.17, 0.4), oob=squish) +
    theme_void() +
      theme(legend.position = "none",
                  legend.title = element_blank(),
            plot.margin = unit(c(0.1, 0.1, 0.1, 0.1), "cm"),
...
lh_ageeffect_plots <- lapply(c("ARCL", "CSTL" ,"ILFL", "IFOL", "SLFL", "pARCL", "VOFL"), plot_ageeffects, dataset="HCPD")
rh_ageeffect_plots <- lapply(c("ARCR", "CSTR", "ILFR", "IFOR", "SLFR", "pARCR", "VOFR"), plot_ageeffects, dataset="HCPD")
...
```{r callosum HCPD age effects, fig.height = 20, fig.width = 15}
# callosum bundles
# plot age effects for each tract
lh_ageeffect_callosum_plots <- lapply(c("COrbL", "CAntFrL" ,"CSupFrL", "CMotL", "CSupParL", "CPostParL", "CTempL", "COccL"), plot_ageeffects, dataset="HCPD")
rh_ageeffect_callosum_plots <- lapply(c("COrbR", "CAntFrR" ,"CSupFrR", "CMotR", "CSupParR", "CPostParR", "CTempR", "COccR"), plot_ageeffects, dataset="HCPD")
...
ggsave(paste0(png_dir, "/", "HCPD", "/","dev_callosum.png"), age_effect_callosum_plots_HCPD, height = 20, width = 15, units = "in")
...
lh_by_region <- rbind(COrbL_deveffect_HCPD, CAntFrL_deveffect_HCPD, 
                      CSupFrL_deveffect_HCPD, CMotL_deveffect_HCPD, 
                      CSupParL_deveffect_HCPD, CPostParL_deveffect_HCPD, 
                      CTempL_deveffect_HCPD, COccL_deveffect_HCPD, 
                      ARCL_deveffect_HCPD, IFOL_deveffect_HCPD, SLFL_deveffect_HCPD) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))

length(which(is.na(lh_by_region$regional_mean_ageeffect)))  

# lh all
lh_by_region_all <- rbind(COrbL_deveffect_HCPD, CAntFrL_deveffect_HCPD, 
                      CSupFrL_deveffect_HCPD, CMotL_deveffect_HCPD, 
                      CSupParL_deveffect_HCPD, CPostParL_deveffect_HCPD, 
                      CTempL_deveffect_HCPD, COccL_deveffect_HCPD, 
                      ARCL_deveffect_HCPD, IFOL_deveffect_HCPD, SLFL_deveffect_HCPD,
                      CSTL_deveffect_HCPD, ILFL_deveffect_HCPD, pARCL_deveffect_HCPD, VOFL_deveffect_HCPD) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))

length(which(is.na(lh_by_region_all$regional_mean_ageeffect)))  

# lh callosum + most tracts minus pARC and VOF
lh_by_region_subset <- rbind(COrbL_deveffect_HCPD, CAntFrL_deveffect_HCPD, 
                      CSupFrL_deveffect_HCPD, CMotL_deveffect_HCPD, 
                      CSupParL_deveffect_HCPD, CPostParL_deveffect_HCPD, 
                      CTempL_deveffect_HCPD, COccL_deveffect_HCPD, 
                      ARCL_deveffect_HCPD, IFOL_deveffect_HCPD, SLFL_deveffect_HCPD,
                      CSTL_deveffect_HCPD, ILFL_deveffect_HCPD) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))

length(which(is.na(lh_by_region_subset$regional_mean_ageeffect)))  

# callosum + tracts of interest only
rh_by_region <- rbind(COrbR_deveffect_HCPD, CAntFrR_deveffect_HCPD, 
                      CSupFrR_deveffect_HCPD, CMotR_deveffect_HCPD, 
                      CSupParR_deveffect_HCPD, CPostParR_deveffect_HCPD, 
                      CTempR_deveffect_HCPD, COccR_deveffect_HCPD, 
                      ARCR_deveffect_HCPD, IFOR_deveffect_HCPD, SLFR_deveffect_HCPD) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))

length(which(is.na(rh_by_region$regional_mean_ageeffect))) 


rh_by_region_all <- rbind(COrbR_deveffect_HCPD, CAntFrR_deveffect_HCPD, 
                      CSupFrR_deveffect_HCPD, CMotR_deveffect_HCPD, 
                      CSupParR_deveffect_HCPD, CPostParR_deveffect_HCPD, 
                      CTempR_deveffect_HCPD, COccR_deveffect_HCPD, 
                      ARCR_deveffect_HCPD, IFOR_deveffect_HCPD, SLFR_deveffect_HCPD,
                      CSTR_deveffect_HCPD, ILFR_deveffect_HCPD, pARCR_deveffect_HCPD, VOFR_deveffect_HCPD) %>% group_by(region) %>% 
  summarise(regional_mean_ageeffect = mean(mean_age_effect, na.rm=T))
...
rh_by_region_subset <- rbind(COrbL_deveffect_HCPD, CAntFrL_deveffect_HCPD, 
                      CSupFrL_deveffect_HCPD, CMotL_deveffect_HCPD, 
                      CSupParL_deveffect_HCPD, CPostParL_deveffect_HCPD, 
                      CTempL_deveffect_HCPD, COccL_deveffect_HCPD, 
                      ARCL_deveffect_HCPD, IFOL_deveffect_HCPD, SLFL_deveffect_HCPD,
...
ggsave(paste0(png_dir, "/", "HCPD", "/","dev_aggregated.png"), agg_tracts_interest , height = 10, width = 10, units = "in")

agg_all <- plot_aggregated_maps(lh_by_region_all, rh_by_region_all)
ggsave(paste0(png_dir, "/", "HCPD", "/","dev_all.png"), agg_all , height = 10, width = 10, units = "in")

agg_subset <- plot_aggregated_maps(lh_by_region_subset, rh_by_region_subset)
ggsave(paste0(png_dir, "/", "HCPD", "/","dev_aggregated_subset.png"), agg_subset, height = 10, width = 10, units = "in")
...
lh_SAranks <- lapply(lh_names, merge_SAaxis, "all_regions", dataset="HCPD")
lh_SAranks_endpoints <- lapply(lh_names, merge_SAaxis, "endpoints_only", dataset="HCPD")
 
rh_names <- c("ARCR", "CSTR" ,"ILFR", "IFOR", "SLFR", "pARCR", "VOFR", "COrbR", "CAntFrR" ,"CSupFrR", "CMotR", "CSupParR", "CPostParR", "CTempR", "COccR")
rh_SAranks <- lapply(rh_names, merge_SAaxis, "all_regions", dataset="HCPD")
rh_SAranks_endpoints <- lapply(rh_names, merge_SAaxis, "endpoints_only", dataset="HCPD")
...
cor.test(abs_diffs_all$mean_SA_diff, abs_diffs_all$age_effect_diff) # nothing
...
lh_agg_final <- merge(lh_by_region_subset, lh_glasser_labels, by = "region")
...
rh_agg_final <- merge(rh_by_region_subset, rh_glasser_labels, by = "region")
...
cor.test(t1t2_ageeffect_hcpd$data, hcpd_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = 0.15, p = 0.02 spearman (all)
 

cor.test(t1t2_mean_hcpd$data, hcpd_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = 0.28, p = 0.000009 spearman (all)
cor.test(t1t2_roc_hcpd$data, hcpd_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = 0.14, p = 0.02 spearman
cor.test(t1t2_maxslopeage_hcpd$data, hcpd_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # meh

cor.test(alff_pnc$data, hcpd_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = -0.04, p =NS
cor.test(alff_decline_pnc$data, hcpd_agg_ageeffects$regional_mean_ageeffect, method = 'spearman') # r = 0.21, pval = 0.0019
...
perm.sphere.p(x = hcpd_agg_ageeffects$regional_mean_ageeffect, y = t1t2_ageeffect_hcpd$data, perm.id = perm.id.full, corr.type = "spearman")    
perm.sphere.p(x = hcpd_agg_ageeffects$regional_mean_ageeffect, y = t1t2_mean_hcpd$data, perm.id = perm.id.full, corr.type = "spearman")  
perm.sphere.p(x = hcpd_agg_ageeffects$regional_mean_ageeffect, y = t1t2_roc_hcpd$data, perm.id = perm.id.full, corr.type = "spearman")   
perm.sphere.p(x = hcpd_agg_ageeffects$regional_mean_ageeffect, y = t1t2_maxslopeage_hcpd$data, perm.id = perm.id.full, corr.type = "spearman")   

perm.sphere.p(x = hcpd_agg_ageeffects$regional_mean_ageeffect, y = alff_pnc$data, perm.id = perm.id.full, corr.type = "spearman")   
perm.sphere.p(x = hcpd_agg_ageeffects$regional_mean_ageeffect, y = alff_decline_pnc$data, perm.id = perm.id.full, corr.type = "spearman")   

---
title: 'Covariate Analyses: envSES'
author: "Audrey Luo"
...
# envSES main effect
```{r load envSES main}
HBN_envSES_main <- read.csv("/cbica/projects/luo_wm_dev/output/HBN/tract_profiles/GAM/dti_md/HBN_GAM_envSES_maineffects.csv")
PNC_envSES_main <- read.csv("/cbica/projects/luo_wm_dev/output/PNC/tract_profiles/GAM/dti_md/PNC_GAM_envSES_maineffects.csv")
```

```{r fdr envSES main}
envSES_main_fdr <- lapply(list(HBN_envSES_main, PNC_envSES_main), sig_nodes, pval_var="GAM.cov.pvalue")
names(envSES_main_fdr) <- c("HBN", "PNC")
# HBN: 0/2200 = 0% have significant main effect of envSES 
# PNC: 22/2200 = 22.45% have significant main effect of envSES 

range(envSES_main_fdr$HBN$GAM.cov.tvalue)
range(envSES_main_fdr$PNC$GAM.cov.tvalue)
which(envSES_main_fdr$HBN$significant.fdr==1)
```
```{r format envSES effect dfs}
# format envSES effect df's
format_envSESeffect <- function(df) {
...
envSESeffect_dfs <- lapply(envSES_main_fdr, format_envSESeffect)
```

1) envSES effect plot: GAM.cov.tvalue in y-axis, node position for each tract in x-axis
```{r plot envSES effect function}

# function for plotting envSES effect
plot_envSES_effect_clipEnds <- function(tract, scalar, envSESeffect_measure, color1, color2, color3, clipEnds, ylim1, ylim2) {
  # envSESeffect_measure = GAM.smooth.AdjRsq or GAM.smooth.partialR2

    HBN <- envSESeffect_dfs$HBN %>% filter(tract_label == {{tract}}) %>% filter(nodeID > (clipEnds-1) & nodeID < (99-clipEnds+1)) %>%
      mutate(Dataset = "HBN")
    PNC <- envSESeffect_dfs$PNC %>% filter(tract_label == {{tract}}) %>% filter(nodeID > (clipEnds-1) & nodeID < (99-clipEnds+1)) %>%
      mutate(Dataset = "PNC")
   
    # NA out color/fill aes if adj rsq = 0 or if p-value doesn't survive FDR correction (makes the color gray)
    if (envSESeffect_measure == "GAM.smooth.AdjRsq" | envSESeffect_measure == "GAM.smooth.partialR2") {
      includes_zero_HBN <- which(HBN[[envSESeffect_measure]]==0 | HBN$GAM.cov.pvalue.fdr > 0.05)
      includes_zero_PNC <- which(PNC[[envSESeffect_measure]]==0 | PNC$GAM.cov.pvalue.fdr > 0.05)
    } else {
      includes_zero_HBN <- which(is.na(HBN[[envSESeffect_measure]]) | HBN[[envSESeffect_measure]]==0 | HBN$GAM.cov.pvalue.fdr > 0.05)
      includes_zero_PNC <- which(is.na(PNC[[envSESeffect_measure]]) | PNC[[envSESeffect_measure]]==0 | PNC$GAM.cov.pvalue.fdr > 0.05)
    }
    ...
    if(str_detect(tract, "Callosum")) {
      plot <- ggplot(data = df, aes(x = nodeID, y = get(envSESeffect_measure), colour = Dataset, fill = Dataset)) +
        ...
        plot <- ggplot(data = df, aes(x = nodeID, y = get(envSESeffect_measure), colour = Dataset, 
...
```{r run envSES effect plotting}
envSESeffect_measure = "GAM.cov.tvalue"
tracts = unique(envSESeffect_dfs$HBN$tract_label)
envSESeffects_plots <- lapply(tracts, plot_envSES_effect_clipEnds, scalar = "dti_md", envSESeffect_measure = envSESeffect_measure, 
                                color1 = color1, color2 = color2, color3 = color3, clipEnds = 3, ylim2 = 8, ylim1 = -8)
names(envSESeffects_plots) <- tracts
```


```{r envSES effects, fig.height = 20, fig.width = 16}
envSESeffects_assoc_plots <- arrange_tract_plots(envSESeffects_plots, "Position on Tract (Node ID)", 0.3)
 
y.grob <- textGrob(expression("envSES Effect (t-value)"), 
                   gp=gpar(col="black", fontsize=20), rot=90)

grid.arrange(arrangeGrob(envSESeffects_assoc_plots, left = y.grob))

ggsave(paste0(png_dir, "envSESeffect_association.png"), grid.arrange(arrangeGrob(envSESeffects_assoc_plots, left = y.grob)), height = 20, width = 16, units = "in")
```

```{r envSES effects callosum, fig.height = 20, fig.width = 10}

envSESeffects_callosum_plots <- arrange_callosum_plots(envSESeffects_plots, "Position on Tract (Node ID)", 0.3)
 
y.grob <- textGrob(expression("envSES Effect (t-value)"), 
                   gp=gpar(col="black", fontsize=20), rot=90)

grid.arrange(arrangeGrob(envSESeffects_callosum_plots, left = y.grob))

ggsave(paste0(png_dir, "envSESeffect_callosum.png"), grid.arrange(arrangeGrob(envSESeffects_callosum_plots, left = y.grob)), height = 20, width = 10, units = "in")
```

2) developmental trajectories separate for highest and lowest envSES scores 
```{r load files for dev traj all nodes}
HBN_smooths <- read.csv("/cbica/projects/luo_wm_dev/output/HBN/tract_profiles/GAM/dti_md/HBN_GAM_agebyenvSES_interaction.csv")
PNC_smooths <- read.csv("/cbica/projects/luo_wm_dev/output/PNC/tract_profiles/GAM/dti_md/PNC_GAM_agebyenvSES_interaction.csv")

HBN.smooths <- format_envSESeffect(HBN_smooths %>% rename(tract_node = tract, age = smooth_var))
PNC.smooths <- format_envSESeffect(PNC_smooths %>% rename(tract_node = tract, age = smooth_var))
```
...
plot_smooths_binned <- function(tract, df, bin_num_nodes, clipEnds) {
  ...
  df_binned <- df %>%
        ...
        group_by(node_position, age, envSES) %>% # removed hemi
        summarise(
          mean_est = mean(fitted, na.rm = TRUE)
        )
  
  plot <- ggplot(df_binned, aes(x = age, y = mean_est, linetype = envSES, group = interaction(envSES, node_position))) +
...
filename <- "fitted_envSES_callosum"
